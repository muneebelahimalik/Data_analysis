...
class DeltaRobot:
    ...
    def __init__(self):
        # Physical parameters
        self.R_base = 180.0      # Base radius (mm)
        self.R_ee = 60.0         # End-effector radius (mm)  
        self.L_upper = 71.0      # Upper arm length (mm)
        self.L_forearm = 177.5   # Forearm length (mm)
        
        # Joint limits
        self.theta_min = 25.0    # degrees
        self.theta_max = 80.0    # degrees
        
        # Leg angles (120° apart)
        self.leg_angles = np.array([0, 120, 240])
        
        # Target specs
        self.workspace_diameter = 320.0
        self.workspace_height = 200.0
    ...
    def forward_kinematics(self, theta1, theta2, theta3):
        ...
        for i, (theta, leg_angle) in enumerate(zip(thetas, self.leg_angles)):
            ...
            # Base attachment point (on base circle)
        ...
        # EE attachment points (on smaller circle, same angles as base)
        ee_attach = []
        ...
        # Solve for EE position using iterative refinement
        x_ee = np.mean(elbows[:, 0])
        y_ee = np.mean(elbows[:, 1])
        
        # Refine z position using distance constraints
        for _ in range(5):  # Iterate for convergence
            r_xy = np.sqrt((elbows[0, 0] - x_ee - ee_attach[0, 0])**2 + 
                           (elbows[0, 1] - y_ee - ee_attach[0, 1])**2)
            
            z_offset = np.sqrt(max(0, self.L_forearm**2 - r_xy**2))
            z_ee = elbows[0, 2] - z_offset
            
            # Refine XY using constraint from leg 1
            target_r = np.sqrt(max(0, self.L_forearm**2 - (elbows[0, 2] - z_ee)**2))
            curr_r = np.sqrt((elbows[0, 0] - x_ee - ee_attach[0, 0])**2 + 
                             (elbows[0, 1] - y_ee - ee_attach[0, 1])**2)
            
            if curr_r > 1e-6:
                scale = target_r / curr_r if target_r > 0 else 0
                x_ee = elbows[0, 0] - ee_attach[0, 0] - (elbows[0, 0] - x_ee - ee_attach[0, 0]) * scale
                y_ee = elbows[0, 1] - ee_attach[0, 1] - (elbows[0, 1] - y_ee - ee_attach[0, 1]) * scale
    ...
    def inverse_kinematics(self, x, y, z):
        ...
        for leg_angle in self.leg_angles:
            ...
            # EE attachment (relative to EE center)
            ...
            if r_total > max_reach + 1e-6 or r_total < min_reach - 1e-6:
                return None
            
            # Law of cosines: find angle at base
            cos_angle = (self.L_upper**2 + r_total**2 - self.L_forearm**2) / (2 * self.L_upper * r_total)
            cos_angle = np.clip(cos_angle, -1, 1)
            
            alpha = np.arccos(cos_angle)
            beta = np.arctan2(-dz, r_xy)  # Negative because Z is downward
            
            # Elbow-down configuration
            theta = beta + alpha
            theta_deg = np.rad2deg(theta)
            
            # Check joint limits with small tolerance
            if theta_deg < self.theta_min - 1e-6 or theta_deg > self.theta_max + 1e-6:
                return None
...
def generate_all_figures():
    ...
    print(f"Base Radius (Rb):       {robot.R_base} mm")
    print(f"End-Effector Radius (Re): {robot.R_ee} mm")
    print(f"Upper Arm Length (Lu):   {robot.L_upper} mm")
    print(f"Forearm Length (Lf):     {robot.L_forearm} mm")
    print(f"Joint Limits:            [{robot.theta_min}°, {robot.theta_max}°]")
    print(f"\nTARGET WORKSPACE:")
    print(f"Diameter:  {robot.workspace_diameter} mm")
    print(f"Depth:     {robot.workspace_height} mm")
    
    # Analyze actual reachable workspace
    print("\nANALYZING ACTUAL REACHABLE WORKSPACE...")
    theta_range = np.linspace(robot.theta_min, robot.theta_max, 15)
    workspace_pts = []
    for t1 in theta_range:
        for t2 in theta_range:
            for t3 in theta_range:
    ...
    if len(workspace_pts) > 0:
        print(f"Reachable workspace XY extent: ±{np.max(np.abs(workspace_pts[:,:2])):.1f} mm")
        print(f"Reachable workspace Z extent: {workspace_pts[:,2].min():.1f} to {workspace_pts[:,2].max():.1f} mm")
        print(f"Vertical span: {workspace_pts[:,2].max() - workspace_pts[:,2].min():.1f} mm")
